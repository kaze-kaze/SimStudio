...
New runs record Workbench mesh shape metrics and global averaged/unaveraged stress hotspots. Missing or inconsistent shape evidence cannot pass, and shape errors cannot be waived. Shape warnings require a target-specific review bound to both the exact RST and raw quality-record hash; the original warning remains visible. See [quality evidence and review format](../skills/ansys-design-study/references/quality-evidence.md). Stress diagnostics identify extrema and preserve element-local value order; extraction success is not a singularity approval.
