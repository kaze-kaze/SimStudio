...
| Independent accuracy and equal-budget comparison | Complete-holdout, false-safe, model commitment and comparison-completeness tests | Offline verified; real accuracy/cost comparison NOT_RUN |
...
| Cross-platform CI and single-run compatibility | Ruff, 471 passed / 15 skipped locally; Linux/Windows jobs configured | Local verified; hosted CI NOT_RUN |
...
- Local full suite: 471 passed, 15 skipped. Skips are real ANSYS integration tests, not passes.
...
- A second dependency environment resolved build123d 0.13.0, NumPy 2.5.3, SciPy 1.18.1 and
  scikit-learn 1.9.1; its CAD, sampling, surrogate and numerical-reference tests passed (58 tests).
