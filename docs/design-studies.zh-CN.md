...
新运行会保存 Workbench 网格形状指标，以及全模型平均和未平均应力热点。缺失或矛盾的形状证据不能通过，真实网格错误不能被复核覆盖。网格警告需要按目标审查，并同时绑定确切 RST 和原始质量记录的哈希；原始警告始终保留。格式见[质量证据与复核](../skills/ansys-design-study/references/quality-evidence.md)。应力诊断保留单元内部值的原始顺序，不把数据提取成功当作奇异性审查通过。
