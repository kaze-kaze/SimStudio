...
def export(archive: Path, target: Path) -> None:
    ...
    _, suite, tests = public_junit(junit)
    record(junit)
    ...
    document = {
        ...
        "environment": {
            ...
            "source": "docs/reports/mechanical-test-2026-09-06.md",
        },
        "suite": suite,
    ...
    published = [out / n for n in ("summary.json", "cases.json")]
...
def check(target: Path) -> dict:
    ...
    suite = summary["suite"]
    if (len(cases) != 9 or suite["tests"] != len(cases)
        ...
        raise ValueError("Public case summary differs from the recorded nine-case baseline")
